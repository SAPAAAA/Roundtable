...
CREATE TABLE "Principal" (
    ...
    "role"       "PrincipalRole" NOT NULL -- Enforces single role type
);
...
-- Sets the updatedAt timestamp on update operations
...
-- Updates the comment count on the parent Post when a Comment is added or deleted
...
RETURNS TRIGGER AS $$
DECLARE
    media_limit CONSTANT INTEGER := 3; -- Max media items per post
...
-- Ensures a Principal cannot be both a RegisteredUser and an Admin simultaneously
...
BEGIN
    ...
    ELSIF TG_TABLE_NAME = 'Admin' THEN
        ...
        END IF;
    END IF;
    RETURN NEW; -- Allow insert if no conflict
END;
$$ LANGUAGE plpgsql;
...
-- Principals (10 records: first 5 users, next 5 admins)
...
-- Admins (5 records for the 5 ‘admin’ principals)
...
-- Comments (20 records, including replies)
...
INSERT INTO "Notification" ("notificationId","recipientAccountId","triggeringPrincipalId","type","sourceUrl","content") VALUES
...
-- Messages (20 records)
INSERT INTO "Message" ("messageId","senderPrincipalId","recipientAccountId","subject","body") VALUES
